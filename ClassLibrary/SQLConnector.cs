...
namespace ClassLibrary
{
    ...
    public class DataMapper
    {
        ...
        public int GetTeamPoints(string team)
        {
            ...
            // Construct the SQL query to select team points from the database
            string query = "SELECT Points FROM TeamInfo WHERE TeamName = @Team";
            ...
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                ...
                try
                {
                    ...
                    // Execute the query and retrieve the points value
                    object result = command.ExecuteScalar();

                    // Check if the result is not null and is convertible to int
                    if (result != null && int.TryParse(result.ToString(), out points))
                    {
                        // Points value successfully retrieved
                        return points;
                    }
                }
                catch (Exception ex)
            ...
            // Return zero if points value retrieval fails or team not found
            return points;
        }
